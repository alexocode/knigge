defmodule Knigge.Options do
  ...
  - `behaviour` the behaviour for which `Knigge` should generate delegations, defaults to `__MODULE__`
  - `config_key` the configuration key from which `Knigge` should fetch the implementation, defaults to `__MODULE__` and is only used then `otp_app` is passed
